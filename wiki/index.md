# Wiki Index

This is the navigation entrypoint for the wiki.

## Overview

- [overview](./overview.md): top-level orientation for the current wiki

## Sources

- [2026-04-06-karpathy-llm-wiki](./sources/2026-04-06-karpathy-llm-wiki.md): seed source page for the Karpathy-inspired pattern
- [2026-04-06-public-course-workstream](./sources/2026-04-06-public-course-workstream.md): source page for the 공개과정 premium open-course workstream
- [2026-04-07-public-course-public-launch](./sources/2026-04-07-public-course-public-launch.md): source page for the public launch layer, application flow, and hosting setup of 공개과정
- [2026-04-06-youth-speech-prescription-workstream](./sources/2026-04-06-youth-speech-prescription-workstream.md): source page for the youth adaptation of `어른을 위한 말처방`
- [2026-04-06-ai-education-proposals-workstream](./sources/2026-04-06-ai-education-proposals-workstream.md): source page for the AI education proposal portfolio
- [2026-04-07-samsung-jobcrafting-proposal](./sources/2026-04-07-samsung-jobcrafting-proposal.md): source page for the Samsung REFRAME jobcrafting proposal and proposal-portfolio expansion
- [2026-04-06-instructional-design-workstream](./sources/2026-04-06-instructional-design-workstream.md): source page for the enterprise instructional design workspace
- [2026-04-06-jeon-jongmok-context](./sources/2026-04-06-jeon-jongmok-context.md): source page for the long-form worldview context around Jeon Jong-mok
- [2026-04-06-first-launch-wiki-routine](./sources/2026-04-06-first-launch-wiki-routine.md): source page for the decision to run the wiki loop at first launch instead of a fixed 09:00 schedule
- [2026-04-06-automated-codex-wiki-sync](./sources/2026-04-06-automated-codex-wiki-sync.md): source page for the automatic host-machine sync pattern and shared-use rule
- [2026-04-06-codex-host-wake-timer](./sources/2026-04-06-codex-host-wake-timer.md): source page for waking the host at 08:50 before the 09:00 wiki sync
- [2026-04-07-windows-led-codex-refresh](./sources/2026-04-07-windows-led-codex-refresh.md): source page for restarting Codex at 08:55 so the automation window starts from a fresh app state
- [2026-04-07-tracked-project-access-registry](./sources/2026-04-07-tracked-project-access-registry.md): source page for which tracked projects are actually portable across devices today
- [2026-04-07-requested-source-handoff-workflow](./sources/2026-04-07-requested-source-handoff-workflow.md): source page for how the host machine should provide local-only source text on request
- [2026-04-07-file-based-request-queue](./sources/2026-04-07-file-based-request-queue.md): source page for the repository-native request queue across PCs and Macs

## Entities

- [andrej-karpathy](./entities/andrej-karpathy.md): source author tied to the initial wiki pattern
- [public-course](./entities/public-course.md): premium public course for working instructors built around redesigning a real lecture
- [youth-speech-prescription](./entities/youth-speech-prescription.md): youth-facing adaptation project grounded in adolescent-development research
- [ai-education-proposal-portfolio](./entities/ai-education-proposal-portfolio.md): proposal system packaging AI education into role-specific B2B programs
- [instructional-design-workbench](./entities/instructional-design-workbench.md): delivery repository of guides, decks, and participant assets
- [jeon-jongmok](./entities/jeon-jongmok.md): portfolio anchor connecting B2B execution and B2C meaning-making

## Concepts

- [persistent-wiki-vs-rag](./concepts/persistent-wiki-vs-rag.md): why a maintained wiki differs from repeated query-time retrieval
- [wiki-ingest-query-lint-loop](./concepts/wiki-ingest-query-lint-loop.md): the operating loop that keeps the wiki useful
- [education-product-redesign-through-practice](./concepts/education-product-redesign-through-practice.md): learning framed as improving real working artifacts through practice and feedback
- [reusable-education-operations-assets](./concepts/reusable-education-operations-assets.md): operating assets treated as reusable infrastructure rather than disposable residue
- [dual-track-ai-education-manager-vs-practitioner](./concepts/dual-track-ai-education-manager-vs-practitioner.md): AI education split by organizational leverage point
- [interpretation-centered-education-design](./concepts/interpretation-centered-education-design.md): worldview that starts from interpreting people and situations before teaching technique

## Analyses

- [current-workstream-map-2026-04](./analyses/current-workstream-map-2026-04.md): synthesis of how the current five workstreams fit together as one portfolio map
- [internal-wiki-operations-loop](./analyses/internal-wiki-operations-loop.md): practical answer for how to run this wiki as a first-launch internal operating layer
- [cross-device-source-basis-policy](./analyses/cross-device-source-basis-policy.md): how to read `Original workspace basis` safely on other devices
- [requested-source-handoff-pattern](./analyses/requested-source-handoff-pattern.md): how to fulfill cross-device requests for original text without losing provenance
- [cross-device-request-queue-pattern](./analyses/cross-device-request-queue-pattern.md): how to coordinate durable asks through repo files instead of chat only
